...
func (c *OfferClient) GetOffersByFilter(filter repoangebot.Filter) ([]*repoangebot.Offer, error) {
	...
	if resp.StatusCode >= 300 {
		return []*repoangebot.Offer{}, fmt.Errorf("failed to get offers, status code: %d", resp.StatusCode)
	}

	var offers []*repoangebot.Offer
	if err := json.NewDecoder(resp.Body).Decode(&offers); err != nil {
		return []*repoangebot.Offer{}, fmt.Errorf("failed to decode response: %w", err)
	}
	return offers, nil
}
